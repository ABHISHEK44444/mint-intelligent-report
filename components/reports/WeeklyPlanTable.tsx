...
interface WeeklyPlanTableProps {
    plans: WeeklyPlan[];
    onAddPlan: (plan: Omit<WeeklyPlan, 'id'>) => void;
    canAdd: boolean;
    userId: string;
...
const WeeklyPlanTable: React.FC<WeeklyPlanTableProps> = ({ plans, onAddPlan, canAdd, userId, onUpdatePlan, isManager }) => {
                ...
                <tbody className="bg-white">
                    {plans.map((plan, index) => (
                        <tr key={plan.id} className="hover:bg-slate-50">
                            ...
                            <td className="p-2 border border-slate-300 whitespace-nowrap text-sm text-slate-500">{plan.date}</td>
