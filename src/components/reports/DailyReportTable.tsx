...
interface DailyReportTableProps {
    reports: DailyReport[];
    onAddReport: (report: Omit<DailyReport, 'id' | 'userId'>) => void;
    canAdd: boolean;
...
const DailyReportTable: React.FC<DailyReportTableProps> = ({ reports, onAddReport, canAdd, onUpdateReport, isManager }) => {
    ...
    const [newReport, setNewReport] = useState(getInitialState());
                ...
                <tbody className="bg-white">
                    {reports.map((report, index) => (
                        <tr key={report.id} className="hover:bg-slate-50">
                            ...
                            <td className="p-2 border border-slate-300 whitespace-nowrap text-sm text-slate-500">{new Date(report.date).toLocaleDateString()}</td>
